...
public class AI : MonoBehaviour
{
	...
	public void AILogic()
	{
		...
		if (enemy.num == 2) nextTile = tiles[manager.Index((int)(currentPos.x + 1), (int)currentPos.y)];
		if (enemy.num == 3) nextTile = tiles[manager.Index((int)(currentPos.x - 1), (int)currentPos.y)];
		if (enemy.num == 0) nextTile = tiles[manager.Index((int)currentPos.x, (int)(currentPos.y + 1))];
		if (enemy.num == 1) nextTile = tiles[manager.Index((int)currentPos.x, (int)(currentPos.y - 1))];
		...
		if (nextTile.occupied || currentTile.isIntersection)
		{
			...
			if (nextTile.occupied && !currentTile.isIntersection)
			{
				...
				{
					if (currentTile.down == null) enemy.direction = Vector3.up;
					else enemy.direction = Vector3.down;
				...
				{
					if (currentTile.left == null) enemy.direction = Vector3.right;
					else enemy.direction = Vector3.left;
			...
			if (currentTile.isIntersection)
			{
				...
				if (currentTile.up != null && !currentTile.up.occupied && enemy.num != 0) dist1 = manager.distance(currentTile.up, targetTile);
				if (currentTile.down != null && !currentTile.down.occupied && enemy.num != 1) dist2 = manager.distance(currentTile.down, targetTile);
				if (currentTile.left != null && !currentTile.left.occupied && enemy.num != 3) dist3 = manager.distance(currentTile.left, targetTile);
				if (currentTile.right != null && !currentTile.right.occupied && enemy.num != 2) dist4 = manager.distance(currentTile.right, targetTile);
				...
				*/
				...
				if (min == dist1) enemy.num = 0;
				if (min == dist2) enemy.num = 1;
				if (min == dist3) enemy.num = 3;
				if (min == dist4) enemy.num = 2;
	...
	public void RunLogic()
	{
		...
		// get the next tile according to direction
		if (enemy.direction.x > 0) nextTile = tiles[manager.Index((int)(currentPos.x + 1), (int)currentPos.y)];
		if (enemy.direction.x < 0) nextTile = tiles[manager.Index((int)(currentPos.x - 1), (int)currentPos.y)];
		if (enemy.direction.y > 0) nextTile = tiles[manager.Index((int)currentPos.x, (int)(currentPos.y + 1))];
		if (enemy.direction.y < 0) nextTile = tiles[manager.Index((int)currentPos.x, (int)(currentPos.y - 1))];

		//Debug.Log (enemy.direction.x + " " + enemy.direction.y);
		//Debug.Log (enemy.name + ": Next Tile (" + nextTile.x + ", " + nextTile.y + ")" );
		...
		if (nextTile.occupied || currentTile.isIntersection)
		{
			...
			if (currentTile.isIntersection)
			{
				...
				if (currentTile.up != null && !currentTile.up.occupied && !(enemy.direction.y < 0)) availableTiles.Add(currentTile.up);
				if (currentTile.down != null && !currentTile.down.occupied && !(enemy.direction.y > 0)) availableTiles.Add(currentTile.down);
				if (currentTile.left != null && !currentTile.left.occupied && !(enemy.direction.x > 0)) availableTiles.Add(currentTile.left);
				if (currentTile.right != null && !currentTile.right.occupied && !(enemy.direction.x < 0)) availableTiles.Add(currentTile.right);

				int rand = Random.Range(0, availableTiles.Count);
				chosenTile = availableTiles[rand];
				enemy.direction = Vector3.Normalize(new Vector3(chosenTile.x - currentTile.x, chosenTile.y - currentTile.y, 0));
				//Debug.Log (enemy.name + ": Chosen Tile (" + chosenTile.x + ", " + chosenTile.y + ")" );
			}
	...
	TileManager.Tile GetTargetTilePerEnemy()
	{
		...
		switch (Enemy.thisEnemyType)
		{
			...
			case "pinky":   // target = pacman + 4*pacman's direction (4 steps ahead of pacman)
				//dir = Move.direction;//target.GetComponent<Move>().getDir();
				...
				// read about it here: http://gameinternals.com/post/2072558330/understanding-pac-man-enemy-behavior
